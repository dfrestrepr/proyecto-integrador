...
def plot_clusters_bokeh(list_x, list_y, list_pais, k, etiquetas, grados_pertenencia, 
    ...
    ### PLoteo cada cluster
    for i in range(k):
        source = ColumnDataSource(data={'x':list_x[np.where(etiquetas==i)], 'y':list_y[np.where(etiquetas==i)], 'cliente':list_pais[np.where(etiquetas==i)],'grados_p':grados_pertenencia[np.where(etiquetas==i)],'cluster_id':etiquetas[np.where(etiquetas==i)]})
    ...
    if to_save:
    ### Guardo el resultado
        output_file('/home/david/proyecto-integrador/outputs/ClustersGenerados/cluster_inicial_'+title+'.html')
        save(p)
...
def plot_cluster_bokeh_cambios(X_data_pca, list_x, list_y, list_xv, list_yv, k, 
    ...
    if to_save:
        ### Guardar resultados
        output_file('/home/david/proyecto-integrador/outputs/ClustersGenerados/cluster' + title + '.html')
        save(p)
